...
import {
  ...
  Check, Copy, GitBranch, LogOut, Menu, MessageCircle, Paperclip, Plus, RefreshCw, Search, Server, ShieldCheck, Sparkles, Trash2, Wifi, WifiOff, X
} from "lucide-react";
...
function Flow() {
  ...
  return <><PageTitle eyebrow="链路诊断" title="处理流水" description="从飞书消息到最终回复，直接查看每个处理阶段的数据和断点。" action={<Freshness />} />
    ...
    <section className="latency-summary">{(summary.data?.latencyStages ?? []).map((item: AnyRecord) => <div key={item.key}><span>{item.label}</span><strong>{item.p50Seconds == null ? "暂无" : `P50 ${formatDurationPrecise(item.p50Seconds)}`}</strong><small>{item.p95Seconds == null ? "尚无完整样本" : `P95 ${formatDurationPrecise(item.p95Seconds)} · ${item.count} 个样本`}</small></div>)}</section>
...
function FlowRun({ item, onOpen }: { item: AnyRecord; onOpen(): void }) {
  ...
  return <article className={`flow-run health-${item.health}`}><header><div><strong className="mono">{item.id.slice(0, 8)}</strong><span>{item.bot_display_name} · {item.chat_type === "group" ? "群聊" : "私聊"} · 第 {item.turn_index} 回合 · {relativeTime(item.updated_at)}</span>{item.bottleneck && <small className="bottleneck-hint">主要耗时：{item.bottleneck.label} {formatDurationPrecise(item.bottleneck.durationSeconds)}{item.bottleneck.share != null ? `（${Math.round(item.bottleneck.share * 100)}%）` : ""}</small>}</div><div><StateBadge state={item.state} /><button className="ghost-button" onClick={onOpen}>查看诊断 <ChevronRight size={15} /></button></div></header><div className="flow-lane">{cells.map((cell, index) => <div className={`flow-cell ${item.currentStage === cell.key ? "current" : ""}`} key={cell.key}><div className="flow-cell-head"><span>{cell.title}</span><code>{cell.state}</code></div><p>{cell.body}</p><AttachmentBadges items={cell.attachments} />{cell.meta && <small title={cell.meta}>{cell.meta}</small>}{index < cells.length - 1 && <ArrowRight className="lane-arrow" size={16} />}</div>)}</div></article>;
}
...
function TaskDetail({ user }: { user: AdminUser }) {
  ...
  return <><PageTitle eyebrow="任务详情" title={`任务 ${id.slice(0, 8)}`} description={`${d.chat_type === "p2p" ? "私聊任务" : "群聊任务"} · ${relativeTime(d.created_at)}创建`} action={<StateBadge state={d.state} />} />
    <div className="detail-grid"><section className="panel span-2"><div className="detail-header"><div><span className="label">当前状态</span><h2>{stateLabel[d.state] ?? d.state}</h2><p>最近更新于 {relativeTime(d.updated_at)}，已尝试 {d.attempt} 次。</p></div><div className="action-group">{allowed.map((command) => <button key={command} className={command === "cancel" ? "danger-button" : "secondary-button"} onClick={() => setDialog(command)}>{commandLabel(command)}</button>)}</div></div>
      <div className="fact-grid"><Fact label="机器人" value={d.bot_display_name} /><Fact label="会话" value={d.chat_type === "group" ? (d.chat_name ?? "群聊") : "私聊"} /><Fact label="会话回合" value={`第 ${d.turn_index} 回合`} /><Fact label="Chat ID" value={d.chat_id} /><Fact label="专属工作区" value={workspaceLabel(d)} /><Fact label="请求者" value={`${d.requester_role === "owner" ? "主人" : "成员"} · ${d.requester_id}`} /><Fact label="生命周期判断" value={d.conversation_disposition === "awaiting_followup" ? "等待续聊" : d.conversation_disposition === "complete" ? "会话结束" : "尚未判断"} /><Fact label="续聊截止" value={d.followup_expires_at ? new Date(d.followup_expires_at).toLocaleString("zh-CN") : "—"} /><Fact label="房间版本" value={`room_seq ${d.room_seq}`} /><Fact label="状态版本" value={`revision ${d.revision}`} /></div></section>
      <section className="panel"><PanelHead title="Codex 绑定" subtitle="同一配置恢复线程" />{d.route_mismatch && <div className="inline-alert"><AlertTriangle size={16} />机器人当前绑定 {d.bot_default_executor_id}，本任务保留创建时路由 {d.executor_id}。</div>}<dl className="detail-list"><Detail label="机器人配置执行器" value={d.bot_default_executor_id} /><Detail label="任务实际执行器" value={d.executor_id} /><Detail label="Codex Home" value={d.executor_home_ref} /><Detail label="Profile" value={d.executor_profile} /><Detail label="Thread" value={d.codex_thread_id} /><Detail label="配置指纹" value={d.executor_config_fingerprint} /><Detail label="注意力模型" value={formatModelPolicy(d.attention_model_snapshot, d.attention_reasoning_effort_snapshot)} /><Detail label="正式执行模型" value={formatModelPolicy(d.execution_model_snapshot, d.execution_reasoning_effort_snapshot)} /></dl></section>
      ...
      <section className="panel span-2"><PanelHead title="阶段耗时" subtitle="拆分路由、注意力、正式执行和发送；自动标记主要瓶颈" />{trace.data ? <><div className="stage-timing-grid">{trace.data.stageTimings.map((item: AnyRecord) => <article className={`stage-timing state-${item.state}`} key={item.key}><span>{item.label}</span><strong>{item.durationSeconds == null ? item.state === "running" ? "进行中" : item.state === "skipped" ? "未产生" : "未经过" : formatDurationPrecise(item.durationSeconds)}</strong><small>{item.startedAt ? new Date(item.startedAt).toLocaleTimeString("zh-CN") : "—"} → {item.completedAt ? new Date(item.completedAt).toLocaleTimeString("zh-CN") : "—"}</small>{(item.model || item.effort) && <small>模型 {item.model ?? "继承 Profile"} · {item.effort ?? "默认强度"}</small>}{item.tokenUsage && <small>Token：输入 {item.tokenUsage.inputTokens ?? "—"} · 输出 {item.tokenUsage.outputTokens ?? "—"} · Reasoning {item.tokenUsage.reasoningOutputTokens ?? "—"}</small>}</article>)}</div>{trace.data.bottleneck && <div className="bottleneck-banner"><Clock3 size={17} /><strong>主要瓶颈：{trace.data.bottleneck.label}</strong><span>{formatDurationPrecise(trace.data.bottleneck.durationSeconds)}{trace.data.bottleneck.share != null ? `，约占端到端 ${Math.round(trace.data.bottleneck.share * 100)}%` : ""}</span></div>}</> : <PageLoading compact />}</section>
