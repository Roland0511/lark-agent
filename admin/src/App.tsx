...
import { BotSkillsCard, ChatSkillOverview, TaskSkillSnapshot, WorkerUserSkills } from "./skill-management";
...
export function App({ queryClient }: { queryClient: QueryClient }) {
  ...
  useEffect(() => {
    ...
    stream.addEventListener("change", (event) => {
      ...
      if (change.type === "chat_context") {
        for (const key of ["chat-context", "skill", "task", "flow", "bot"]) void queryClient.invalidateQueries({ queryKey: [key] });
      }
      if (change.type === "worker") {
        for (const key of ["task", "flow", "chat-context", "bot", "incident"]) void queryClient.invalidateQueries({ queryKey: [key] });
      }
      if (change.type === "skill") {
        for (const key of ["skill", "bot", "task", "chat-context", "worker"]) void queryClient.invalidateQueries({ queryKey: [key] });
      }
...
function Bots({ user }: { user: AdminUser }) {
  ...
  return <><PageTitle eyebrow="飞书身份" title="机器人" description="先确认接入是否正常，再按需查看角色、路由与长期聊天记忆。" action={<button className="primary-button" onClick={() => setAdding(true)}><Plus size={17} />添加机器人</button>} />
    {!bots.data ? <PageLoading /> : items.length ? <div className="master-detail-layout bot-master-detail">
      <aside className="master-list-panel" aria-label="机器人列表">
        ...
        <div className="master-list">{items.map((bot: AnyRecord) => {
          ...
          return <button key={bot.id} aria-pressed={selected?.id === bot.id} className={selected?.id === bot.id ? "master-list-item selected" : "master-list-item"} onClick={() => setParams({ bot: bot.id })}>
            <span className="list-item-icon"><Bot size={20} /></span><span><strong>{bot.displayName}</strong><small>{bot.roleInstructions || "通用助理机器人"}</small><small>默认执行器：{executorReference(workerItems, bot.defaultExecutorId, "自动选择")}</small></span><StateBadge state={state} label={state === "online" ? "已启用" : undefined} />
          </button>;
        })}</div>
      ...
      {selected && <BotMasterDetailPane bot={selected} workers={workerItems} user={user} onEdit={() => setEditing(selected)} onRefreshed={() => void bots.refetch()} />}
...
function BotMasterDetailPane({ bot, workers, user, onEdit, onRefreshed }: { bot: AnyRecord; workers: AnyRecord[]; user: AdminUser; onEdit(): void; onRefreshed(): void }) {
  const message = bot.runtime?.[`${bot.id}:message`];
...
export function ChatMemoryTable({ items, onOpen }: { items: AnyRecord[]; onOpen(id: string): void }) {
  ...
  const titles = chatDisplayNames(items);
  return <div className="table-wrap"><table className="chat-memory-table"><thead><tr><th>机器人与聊天</th><th>Chat ID</th><th>Codex Thread</th><th>状态</th><th>固定执行器</th><th>总工作区</th><th>升级后自动压缩</th><th>最后活动</th><th /></tr></thead><tbody>{items.map((item) => { const title = titles.get(item.id) ?? chatDisplayName(item); return <tr key={item.id} onClick={() => onOpen(item.id)}><td><strong>{item.botDisplayName}</strong><small>{title}</small></td><td className="mono" title={item.chatId}>{shortId(item.chatId) ?? "—"}</td><td className="mono" title={item.threadId ?? undefined}>{shortId(item.threadId) ?? "尚未建立"}</td><td><StateBadge state={item.state} label={chatContextStateLabel(item.state)} /></td><td>{item.executorDisplayName ?? item.executorId ?? "首次执行时固定"}{item.executorId && <small className="mono" title={item.executorId}>{shortId(item.executorId)}</small>}</td><td>{item.workspaceRootAlias ?? "首次执行时固定"}</td><td>{Number(item.autoCompactionCount ?? 0)} 次<small>{item.lastCompactedAt ? relativeTime(item.lastCompactedAt) : "尚未观测到"}</small></td><td>{relativeTime(item.lastActivityAt)}</td><td><button className="icon-button" aria-label={`查看 ${title} 的聊天记忆`} onClick={(event) => { event.stopPropagation(); onOpen(item.id); }}><ChevronRight size={17} /></button></td></tr>; })}</tbody></table></div>;
}
...
function ChatMemoryDetailDialog({ id, onClose }: { id: string; onClose(): void }) {
  ...
  return <Modal title="聊天记忆详情" onClose={onClose}>{detail.isLoading ? <PageLoading compact /> : detail.error ? <ErrorBox error={detail.error} /> : detail.data && <div className="chat-memory-detail"><div className="chat-memory-summary"><div><span>{detail.data.chatType === "group" ? "群聊" : "私聊"}</span><strong>{chatDisplayName(detail.data)}</strong><small>{detail.data.botDisplayName} · {detail.data.botAppId}</small></div><StateBadge state={detail.data.state} label={chatContextStateLabel(detail.data.state)} /></div>{detail.data.blockedReason && <div className="inline-alert"><AlertTriangle size={17} />{detail.data.blockedReason}</div>}<dl className="detail-list"><Detail label="Chat Context ID" value={detail.data.id} /><Detail label="Chat ID" value={detail.data.chatId} />{detail.data.chatType === "p2p" && <Detail label="用户 Open ID" value={detail.data.peerOpenId} />}<Detail label="Codex Thread" value={detail.data.threadId} /><Detail label="固定执行器" value={executorNameWithId(detail.data.executorDisplayName, detail.data.executorId)} /><Detail label="Codex Profile" value={detail.data.executorProfile} /><Detail label="配置指纹" value={detail.data.executorConfigFingerprint} /><Detail label="总工作区别名" value={detail.data.workspaceRootAlias} /><Detail label="聊天工作区" value={detail.data.workspaceKey ? `${detail.data.workspaceRootAlias ?? "<总工作区>"}/${detail.data.botAppId}/chats/${detail.data.workspaceKey}` : null} /><Detail label="最后活动" value={formatDateTime(detail.data.lastActivityAt)} /><Detail label="最近自动压缩" value={formatDateTime(detail.data.lastCompactedAt)} /></dl><section className="chat-compaction-history"><strong>Codex 自动压缩记录</strong><p>自动压缩只减少上下文占用，不会更换 Thread 或聊天工作区。</p>{detail.data.compactions?.length ? <div>{detail.data.compactions.map((item: AnyRecord) => <article key={item.id}><div><span>{formatDateTime(item.occurredAt)}</span><strong className="mono">Turn {shortId(item.turnId) ?? "—"}</strong></div><small className="mono">Thread {shortId(item.threadId) ?? "—"}{item.itemId ? ` · Item ${shortId(item.itemId)}` : ""}</small></article>)}</div> : <Empty icon={<FileClock />} title="尚未观测到自动压缩" text="Codex 触发原生上下文压缩后，审计记录会显示在这里。" />}</section></div>}</Modal>;
}
...
function ChatMemoryWorkspace({ user }: { user: AdminUser }) {
  ...
  const visibleTitles = chatDisplayNames(visibleItems);
  ...
  return <><div className="workspace-breadcrumb"><NavLink to="/bots"><ArrowLeft size={16} />返回机器人</NavLink><span>机器人 / {bot?.displayName ?? "…"} / 聊天记忆</span></div>
    ...
    <div className={`chat-memory-workspace ${contextId ? "has-selection" : "showing-list"}`}>
      <aside className="chat-memory-list-panel">
        <div className="chat-memory-search"><Search size={17} /><input aria-label="搜索聊天记忆" placeholder="搜索联系人、群名或 ID" value={search} onChange={(event) => setSearch(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") setQ(search.trim()); }} /><button aria-label="执行搜索" onClick={() => setQ(search.trim())}><ArrowRight size={16} /></button></div>
        ...
        {contexts.isLoading ? <PageLoading compact /> : contexts.error ? <ErrorBox error={contexts.error} /> : visibleItems.length ? <div className="chat-memory-list">{visibleItems.map((item: AnyRecord) => <button key={item.id} aria-pressed={contextId === item.id} className={contextId === item.id ? "selected" : ""} onClick={() => navigate(`/bots/${botId}/chat-memory/${item.id}`)}><span className={`chat-avatar state-${item.state}`}>{item.chatType === "group" ? <MessagesSquare size={19} /> : <UserRound size={19} />}</span><span><strong>{visibleTitles.get(item.id) ?? chatDisplayName(item)}</strong><small><StateText state={item.state} /> · {relativeTime(item.lastActivityAt)}</small></span><ChevronRight size={17} /></button>)}</div> : <Empty icon={<MessageCircle />} title="没有匹配的聊天" text="调整状态或搜索条件后再试。" />}
      </aside>
...
function ChatMemoryWorkspaceDetail({ id, botId, user }: { id: string; botId: string; user: AdminUser }) {
  ...
  const defaultChecks = [
    ...
    { key: "executor", label: "执行器", state: "pending", detail: executorNameWithId(d.executorDisplayName, d.executorId) ?? "尚未固定" },
...
function Overview() {
  ...
  return <><PageTitle eyebrow="实时态势" title="运行总览" description="先看系统结论，再按运行对象和最近任务逐层下钻。" action={<Freshness />} />
    ...
    <section className="panel recent-task-panel"><PanelHead title="最近任务" subtitle="低密度浏览当前进展，点击任务查看八阶段链路" link="/tasks" />{recentTasks.data?.items?.length ? <div className="recent-task-list">{recentTasks.data.items.map((item: AnyRecord) => <NavLink to={`/tasks/${item.id}`} key={item.id}><span><strong>{item.summary || item.latest_signal_content || `第 ${item.turn_index} 回合消息处理`}</strong><small>{item.bot_display_name} · {chatDisplayName(item)} · 任务 {shortId(item.id)}</small></span><StateBadge state={item.state} /><span><small>执行器</small><strong>{item.executor_display_name ?? item.executor_id ?? "等待分配"}</strong></span><time>{relativeTime(item.updated_at)}</time><ChevronRight size={17} /></NavLink>)}</div> : <Empty icon={<Inbox />} title="还没有任务" text="机器人收到有效消息后，最近任务会显示在这里。" />}</section>
  </>;
}
...
function FlowRun({ item, defaultOpen, onOpen }: { item: AnyRecord; defaultOpen: boolean; onOpen(): void }) {
  ...
  const cells = [
    ...
    { key: "routing", title: "路由", state: item.chat_context_state === "blocked" ? "blocked" : item.executor_id ? "ready" : item.state, body: item.chat_context_state === "blocked" ? "固定环境核验未通过" : item.executor_id ? `已路由至 ${item.executor_display_name ?? item.executor_id}` : "等待执行器", meta: [item.executor_id, workspaceLabel(item), item.executor_profile].filter(Boolean).join(" · ") || null },
...
function Tasks() {
  ...
  return <><PageTitle eyebrow="工作流" title="任务中心" description="任务、收件、发件与链路诊断在同一处按需展开。" action={<Freshness />} />
    ...
    {view === "tasks" ? <><div className="filterbar"><div className="search"><Search size={18} /><input aria-label="搜索任务" placeholder="联系人、群名、任务或会话 ID" defaultValue={q} onKeyDown={(e) => { if (e.key === "Enter") setParams({ view: "tasks", ...(state && { state }), ...(bot && { bot }), ...(chatContextId && { chatContextId }), ...((e.currentTarget.value) && { q: e.currentTarget.value }) }); }} /></div><select aria-label="机器人" value={bot} onChange={(e) => setParams({ view: "tasks", ...(state && { state }), ...(e.target.value && { bot: e.target.value }), ...(q && { q }), ...(chatContextId && { chatContextId }) })}><option value="">全部机器人</option>{bots.data?.items?.map((item: AnyRecord) => <option key={item.id} value={item.id}>{item.displayName}</option>)}</select><select aria-label="任务状态" value={state} onChange={(e) => setParams({ view: "tasks", ...(e.target.value && { state: e.target.value }), ...(bot && { bot }), ...(q && { q }), ...(chatContextId && { chatContextId }) })}><option value="">全部状态</option>{Object.entries(stateLabel).map(([key, label]) => <option key={key} value={key}>{label}</option>)}</select></div>
      ...
      <article className="panel table-panel">{!tasks.data ? <PageLoading compact /> : visibleTasks.length ? <div className="table-wrap"><table><thead><tr><th>任务</th><th>机器人</th><th>状态</th><th>当前阶段</th><th>专属工作区</th><th>执行器</th><th>更新时间</th><th /></tr></thead><tbody>{visibleTasks.map((task: AnyRecord) => <tr key={task.id} role="link" tabIndex={0} onKeyDown={(event) => { if (event.key === "Enter") navigate(`/tasks/${task.id}`); }} onClick={() => navigate(`/tasks/${task.id}`)}><td><strong className="mono">{shortId(task.id)}</strong><small>{chatDisplayName(task)} · 第 {task.turn_index} 回合</small></td><td>{task.bot_display_name}</td><td><StateBadge state={task.state} /></td><td>{task.current_stage ? flowStageLabel[task.current_stage] ?? task.current_stage : task.state === "completed" ? "飞书回复" : "按需诊断"}</td><td>{workspaceLabel(task)}</td><td>{task.executor_display_name ?? task.executor_id ?? "等待分配"}{task.executor_id && <small className="mono" title={task.executor_id}>{shortId(task.executor_id)}</small>}</td><td>{relativeTime(task.updated_at)}</td><td><button className="icon-button" aria-label={`查看任务 ${shortId(task.id)} 的详情`}><ChevronRight size={17} /></button></td></tr>)}</tbody></table></div> : <Empty icon={<Inbox />} title="没有符合条件的任务" text="调整筛选条件后再试。" />}</article></> : <Flow />}
  </>;
}
...
function TaskDetail({ user }: { user: AdminUser }) {
  ...
  return <><div className="workspace-breadcrumb"><NavLink to="/tasks"><ArrowLeft size={16} />返回任务中心</NavLink><span>任务中心 / 任务 {shortId(id)}</span></div>
    <PageTitle eyebrow="任务详情" title={`任务 ${shortId(id)}`} description={`${d.bot_display_name} · ${chatDisplayName(d)} · 第 ${d.turn_index} 回合 · ${relativeTime(d.created_at)}创建`} action={<StateBadge state={d.state} />} />
    ...
    <section className="task-stage-panel"><div className="task-stage-heading"><div><strong>{taskStageProgress(d, trace.data).current + 1} / 8 · {blocked ? "已暂停" : stateLabel[d.state] ?? d.state}</strong><span>最近更新 {relativeTime(d.updated_at)} · 已尝试 {d.attempt} 次</span></div><div className="action-group">{allowed.map((command) => <button key={command} className={command === "cancel" ? "danger-button" : "secondary-button"} onClick={() => setDialog(command)}>{commandLabel(command)}</button>)}</div></div>
      ...
      <div className="task-context-strip"><span><Bot size={15} />{d.bot_display_name}</span><span>{d.chat_type === "group" ? <MessagesSquare size={15} /> : <UserRound size={15} />}{chatDisplayName(d)}</span><span>第 {d.turn_index} 回合</span><span><HardDrive size={15} />{workspaceLabel(d)}</span></div>
    </section>
...
function Workers({ user }: { user: AdminUser }) {
  ...
  const [editingAlias, setEditingAlias] = useState<AnyRecord | null>(null);
  const [adding, setAdding] = useState(false);
...
export function WorkerAliasDialog({ worker, user, onClose }: { worker: AnyRecord; user: AdminUser; onClose(): void }) {
  const queryClient = useQueryClient();
  const [alias, setAlias] = useState(worker.display_alias ?? "");
  const trimmedAlias = alias.trim();
  const invalidControlCharacter = /[\u0000-\u001f\u007f-\u009f]/u.test(trimmedAlias);
  const mutation = useMutation({
    mutationFn: (displayAlias: string | null) => api(`/v1/admin/workers/${worker.executor_id}/display-alias`, { method: "PATCH", body: JSON.stringify({ displayAlias }) }, user),
    onSuccess: () => {
      for (const key of ["worker", "overview", "task", "flow", "chat-context", "bot", "incident"]) void queryClient.invalidateQueries({ queryKey: [key] });
      onClose();
    }
  });
  const unchanged = trimmedAlias === (worker.display_alias ?? "");
  return <Modal title="编辑执行器别名" onClose={onClose}><div className="worker-alias-context"><div><span>当前设备名称</span><strong>{worker.reported_display_name}</strong></div><div><span>完整执行器 ID</span><code>{worker.executor_id}</code></div></div><label className="worker-alias-field" htmlFor="worker-display-alias"><span>控制台别名</span><input id="worker-display-alias" aria-label="控制台别名" autoFocus maxLength={64} value={alias} placeholder={worker.reported_display_name} onChange={(event) => setAlias(event.target.value)} /><small>{alias.length} / 64 个字符；只改变控制台中的显示名称。</small></label>{invalidControlCharacter && <div className="inline-alert"><AlertTriangle size={17} />别名不能包含换行等控制字符。</div>}{mutation.error && <ErrorBox error={mutation.error} />}<div className="modal-actions split-actions">{worker.display_alias ? <button className="ghost-button" disabled={mutation.isPending} onClick={() => mutation.mutate(null)}><RotateCcw size={15} />恢复设备名称</button> : <span />}<div><button className="ghost-button" disabled={mutation.isPending} onClick={onClose}>取消</button><button className="primary-button" disabled={mutation.isPending || !trimmedAlias || trimmedAlias.length > 64 || invalidControlCharacter || unchanged} onClick={() => mutation.mutate(trimmedAlias)}>{mutation.isPending ? "保存中…" : "保存别名"}</button></div></div></Modal>;
}
...
function executorNameWithId(displayName?: string | null, executorId?: string | null): string | null {
  if (!executorId) return null;
  if (!displayName || displayName === executorId) return executorId;
  return `${displayName}（${executorId}）`;
}
function executorReference(workers: AnyRecord[], executorId?: string | null, empty = "—"): string {
  if (!executorId) return empty;
  const displayName = workers.find((worker) => worker.executor_id === executorId)?.display_name;
  return displayName ? `${displayName} · ${shortId(executorId)}` : executorId;
}
