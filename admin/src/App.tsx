...
export function ChatMemoryTable({ items, onOpen }: { items: AnyRecord[]; onOpen(id: string): void }) {
  ...
  return <div className="table-wrap"><table className="chat-memory-table"><thead><tr><th>机器人与聊天</th><th>Chat ID</th><th>Codex Thread</th><th>状态</th><th>固定执行器</th><th>总工作区</th><th>升级后自动压缩</th><th>最后活动</th><th /></tr></thead><tbody>{items.map((item) => <tr key={item.id} onClick={() => onOpen(item.id)}><td><strong>{item.botDisplayName}</strong><small>{item.chatType === "group" ? `群聊 · ${item.chatName ?? "未命名群聊"}` : "私聊"}</small></td><td className="mono" title={item.chatId}>{shortId(item.chatId) ?? "—"}</td><td className="mono" title={item.threadId ?? undefined}>{shortId(item.threadId) ?? "尚未建立"}</td><td><StateBadge state={item.state} label={chatContextStateLabel(item.state)} /></td><td>{item.executorDisplayName ?? item.executorId ?? "首次执行时固定"}{item.executorId && <small className="mono" title={item.executorId}>{shortId(item.executorId)}</small>}</td><td>{item.workspaceRootAlias ?? "首次执行时固定"}</td><td>{Number(item.autoCompactionCount ?? 0)} 次<small>{item.lastCompactedAt ? relativeTime(item.lastCompactedAt) : "尚未观测到"}</small></td><td>{relativeTime(item.lastActivityAt)}</td><td><button className="icon-button" aria-label={`查看 ${item.chatName ?? item.chatId} 的聊天记忆`} onClick={(event) => { event.stopPropagation(); onOpen(item.id); }}><ChevronRight size={17} /></button></td></tr>)}</tbody></table></div>;
}
...
function Overview() {
  ...
  return <><PageTitle eyebrow="实时态势" title="运行总览" description="先看系统结论，再按运行对象和最近任务逐层下钻。" action={<Freshness />} />
    ...
    <section className="panel recent-task-panel"><PanelHead title="最近任务" subtitle="低密度浏览当前进展，点击任务查看八阶段链路" link="/tasks" />{recentTasks.data?.items?.length ? <div className="recent-task-list">{recentTasks.data.items.map((item: AnyRecord) => <NavLink to={`/tasks/${item.id}`} key={item.id}><span><strong>{item.summary || item.latest_signal_content || `第 ${item.turn_index} 回合消息处理`}</strong><small>{item.bot_display_name} · {item.chat_type === "p2p" ? "私聊" : "群聊"} · 任务 {shortId(item.id)}</small></span><StateBadge state={item.state} /><span><small>执行器</small><strong>{item.executor_display_name ?? item.executor_id ?? "等待分配"}</strong></span><time>{relativeTime(item.updated_at)}</time><ChevronRight size={17} /></NavLink>)}</div> : <Empty icon={<Inbox />} title="还没有任务" text="机器人收到有效消息后，最近任务会显示在这里。" />}</section>
  </>;
}
