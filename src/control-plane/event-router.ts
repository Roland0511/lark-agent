...
import { adminRoleFor } from "./admin-auth.js";
...
export class EventRouter {
  ...
  async handleMessage(event: LarkMessageEvent): Promise<void> {
    const duplicate = await this.db.selectFrom("processed_events").select("event_id").where("event_id", "=", event.event_id).executeTakeFirst();
    if (duplicate) return;
    ...
    if (event.chat_type === "group" && !this.config.whitelistChatIds.has(event.chat_id)) {
      return;
    }

    const rootMessageId = details.rootId ?? details.messageId;
    const explicitlyActivated = event.chat_type === "p2p" || details.mentions.some((mention) => mention.id === this.config.botAppId);

    let createdTaskId: string | null = null;
    await this.db.transaction().execute(async (trx) => {
      if (event.chat_type === "group") {
        await sql`select pg_advisory_xact_lock(hashtext(${event.chat_id}))`.execute(trx);
      }
      ...
      const marker = await trx.insertInto("processed_events").values({ event_id: event.event_id, event_type: event.type, status: "processed", processed_at: new Date() }).onConflict((conflict) => conflict.column("event_id").doNothing()).returning("event_id").executeTakeFirst();
      if (!marker) return;
      if (!conversation) {
      ...
      let task = await trx
        ...
        .executeTakeFirst();
      if (!task) {
        const policy = await trx.selectFrom("chat_policies").selectAll().where("chat_id", "=", event.chat_id).executeTakeFirst();
        const owner = event.sender_id === this.config.ownerOpenId;
        ...
        task = await trx
          ...
          .values({
            ...
            requested_workspace_alias: previous?.requested_workspace_alias ?? policy?.workspace_alias ?? null,
            resolved_workspace_alias: previous?.resolved_workspace_alias ?? null,
            preferred_executor_id: previous?.executor_id ?? previous?.preferred_executor_id ?? policy?.preferred_executor_id ?? null,
      ...
      await trx
        ...
        .values({
          ...
          sender_role: event.sender_id === this.config.ownerOpenId ? "owner" : "member",
  ...
  async handleCardAction(event: LarkCardActionEvent): Promise<void> {
    const duplicate = await this.db.selectFrom("processed_events").select("event_id").where("event_id", "=", event.event_id).executeTakeFirst();
    if (duplicate) return;
    const marker = await this.db.insertInto("processed_events").values({ event_id: event.event_id, event_type: event.type, status: "processed", processed_at: new Date() }).onConflict((conflict) => conflict.column("event_id").doNothing()).returning("event_id").executeTakeFirst();
    if (!marker) return;
    if (event.operator_id !== this.config.ownerOpenId) throw new AppError("only the owner may use task control cards", 403, "owner_required");
  ...
  private async markDiscarded(eventId: string, eventType: string): Promise<void> {
    await this.db.insertInto("processed_events").values({ event_id: eventId, event_type: eventType, status: "discarded", processed_at: new Date() }).onConflict((conflict) => conflict.column("event_id").doNothing()).execute();
  }
  ...
  private async handleHelp(event: LarkMessageEvent): Promise<void> {
    await this.lark.sendMarkdownToChat(
      event.chat_id,
      [
        `**${this.config.agentDisplayName} 可用指令**`,
        "",
        "- `/帮助` 或 `/help`：查看这份指令列表。",
        "- `/连接控制台`：生成 2 分钟内有效、仅可使用一次的控制台连接；仅主人可用。",
        "",
        `请在与 ${this.config.agentDisplayName} 的私聊中单独发送指令，不要添加其他文字。`
      ].join("\n"),
      `command-help-${sha256(event.event_id).slice(0, 24)}`
    );
    await this.db.insertInto("processed_events").values({ event_id: event.event_id, event_type: event.type, status: "command_help", processed_at: new Date() }).onConflict((conflict) => conflict.column("event_id").doNothing()).execute();
  }

  private async handleAdminConnect(event: LarkMessageEvent): Promise<void> {
    const role = adminRoleFor(this.config, event.sender_id);
    if (!role) {
    ...
    await this.lark.sendMarkdownToChat(
      event.chat_id,
      [
        `**${this.config.agentDisplayName} 控制台 · 身份确认**`,
        "",
        `已确认你的飞书身份。请在 2 分钟内点击：[打开 ${this.config.agentDisplayName} 控制台](${link})`,
        "",
        "该链接仅可使用一次，请勿转发。"
      ].join("\n"),
      `admin-connect-${sha256(event.event_id).slice(0, 24)}`
    );
    await this.db.insertInto("processed_events").values({ event_id: event.event_id, event_type: event.type, status: "admin_login", processed_at: new Date() }).onConflict((conflict) => conflict.column("event_id").doNothing()).execute();
  }
}
