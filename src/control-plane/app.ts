...
import { registerDeviceCommandRoutes } from "./device-command-routes.js";
...
export function buildControlPlane(
  ...
  const app = Fastify({
    ...
    logger: { redact: ["req.headers.authorization", "req.headers.cookie", "req.headers.x-lease-token", "req.headers.x-sync-lease-token", "req.headers.x-snapshot-lease-token", "req.headers.x-upgrade-drain-token", "body.token", "body.leaseToken", "body.appSecret", "body.value", "body.contentBase64", "body.content", "body.text", "body.payload", "body.result", "body.turns", "body.items", "body.threadMetadata", "body.contexts"] }
  });
  ...
  registerDeviceCommandRoutes(app, db, config, adminEvents);
