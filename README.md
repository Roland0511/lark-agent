...
单个附件默认上限为 100MB，单任务去重后的附件总量默认上限为 200MB。下载失败、资源已删除或超限时，其他文本和附件仍会继续处理，任务时间线会记录不可用原因。Runner 将附件原子写入机器人专属工作区的 `.lark-agent/attachments/<message-id>/`，目录权限为 `0700`、文件权限为 `0600`，默认保留 7 天；附件内容不写入 PostgreSQL。相关配置为：
